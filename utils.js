...
export function getSectionListData(data) {
  ...
  for (const dish of data) {
    const { category } = dish;
    const formatedDish = (({ id, title, price }) => ({ id, title, price }))(
      dish
    );

    if (category in categoriesData) {
      categoriesData[category] = [...categoriesData[category], formatedDish];
    } else {
      categoriesData[category] = [formatedDish];
    }
  }

  return Object.entries(categoriesData).map(([title, data]) => ({
    title,
    data,
  }));
